...
public class TowerPlacement : MonoBehaviour
{
    ...
    private void Update()
    {
        if(placingTower)
        {
            ...
            if(Physics.Raycast(ray, out hit, 99, tileLayerMask) && hit.collider.GetComponent<TowerTile>().tower == null)
            {
            ...
            if(Mouse.current.leftButton.isPressed && curSelectedTile != null)
            {
                PlaceTower();
            }
    ...
    void PlaceTower()
    {
        ...
        GameManager.instance.ChangeMoney(towerToPlaceDown.cost, true);
